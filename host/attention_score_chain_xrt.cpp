...
int run_single_tile(const Args& args, xrt::device& device, const xrt::uuid& uuid) {
  ...
  auto score_mask_scale_kernel = xrt::kernel(device, uuid, "score_mask_scale_u55c_kernel");
  ...
  auto q_bo =
      xrt::bo(device, sizeof(std::int8_t) * q_tile.size(), score_mask_scale_kernel.group_id(0));
  auto k_bo =
      xrt::bo(device, sizeof(std::int8_t) * k_tile.size(), score_mask_scale_kernel.group_id(1));
  auto scaled_score_bo =
      xrt::bo(
          device,
          sizeof(float) * score_scaled_expected.size(),
          score_mask_scale_kernel.group_id(2));
  ...
  timings.reserve(run_v_stage ? 4 : 3);
  ...
  timings.push_back({"score_mask_scale_u55c_kernel", run_timed("score+mask+scale kernel", [&]() {
...
int run_tiled_inputs(
  ...
  for (std::size_t set = 0; set < 2; ++set) {
    q_bos.emplace_back(
        device,
        sizeof(std::int8_t) * kQTileElems,
        score_mask_scale_kernel.group_id(0));
    k_bos.emplace_back(
        device,
        sizeof(std::int8_t) * kKTileElems,
        score_mask_scale_kernel.group_id(1));
    scaled_score_bos.emplace_back(
        device,
        sizeof(float) * kScoreTileElems,
        score_mask_scale_kernel.group_id(2));
  }
  ...
  using Clock = std::chrono::steady_clock;
  std::vector<TimedStage> timings = {
      {"score_mask_scale_u55c_kernel", 0.0},
  ...
  for (std::uint32_t query_base = 0; query_base < seq_len; query_base += kScoreRowsPerTile) {
    ...
    std::array<PendingFusedTile, 2> pending_tiles;
    ...
    auto finish_pending_tile = [&](std::size_t set) {
      auto& pending = pending_tiles[set];
      if (!pending.valid) {
        return;
      }
    ...
    for (std::uint32_t key_tile_idx = 0; key_tile_idx < k_chunks; ++key_tile_idx) {
      ...
      pending.run.emplace(score_mask_scale_kernel(
          q_bos[set],
          k_bos[set],
      ...
      if (next_key_tile_idx < k_chunks) {
        ...
        const std::size_t next_set = next_key_tile_idx % 2U;
        finish_pending_tile(next_set);
        preload_k_tile(next_set, next_key_base);
      }
    }

    finish_pending_tile(0);
    finish_pending_tile(1);

    full_row_score_bo.write(full_row_logits.data());
    full_row_score_bo.sync(XCL_BO_SYNC_BO_TO_DEVICE);
    timings[1].milliseconds += run_timed("full-row softmax", [&]() {
    ...
    for (std::uint32_t key_base = 0; key_base < seq_len; key_base += kScoreColsPerTile) {
      ...
      timings[2].milliseconds += run_timed("v weighted sum tile", [&]() {
        return v_weighted_sum_kernel(
            weights_tile_bo,
            v_tile_bo,
            attn_partial_bo,
