...
- U55C-oriented HLS kernels for:
  ...
  - score scaling
  - row-wise softmax
- native XRT host app for the four-kernel chain
...
softmax stay as separate kernels.
...
- `score_masked.txt`: exact integer compare
...
One verified real-card timing run printed:
...
- masking, scaling, and softmax are clean separate stages
- it matches the way the existing repo already exports score tiles
- the chain already maps buffers across multiple HBM banks

## Next Work

- Add vector cases for multiple `query_rows` and `key_cols`
- Add an FPGA benchmark runner that reports latency, tiles/sec, scores/sec, and
  HBM traffic estimates
- Add a software comparison baseline for the same cases
- Implement `softmax @ V`
